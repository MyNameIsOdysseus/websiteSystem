		...
		$.ajax({
			...
			success:function(result){
				...
				for(var a=0;a<result.auditLsit.length;a++){
					//result.auditLsit[a].publisdate=new Date(result.auditLsit[a].publisdate).format("yyyy-MM-dd hh:mm:ss");
					result.auditLsit[a].publisdate=NewDate(result.auditLsit[a].publisdate).format("yyyy-MM-dd hh:mm:ss");
					//result.auditLsit[a].publisdate!=null?result.auditLsit[a].publisdate=result.auditLsit[a].publisdate.substr(0,10):null;
					result.auditLsit[a].xuhao=xia++;
				}
				$("#livetable").empty();
				var xx=new Array("xuhao","auditname","publisdate","sorted","channame","auditid");
